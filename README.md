...
本项目已去掉强制登录限制：**无需登录任何 Claude 账号、无需 OAuth、不会弹出 `/login` 或 API key 批准框**。
只要通过环境变量提供凭证，即可在 Linux 上让 Python 等脚本直接以 headless 模式（`claude -p`）调用。

支持三类凭证，按需用环境变量切换：

**1. 官方 Anthropic API key**
...
**2. 第三方代理 / 自建 Anthropic 协议端点**（任意 key + 自定义 Base URL）
...
**3. OpenAI / Gemini / Grok 兼容端点**（DeepSeek、Ollama、vLLM、代理等）
