...
export function getAnthropicApiKeyWithSource(
  ...
  // Login-restriction removed: treat any ANTHROPIC_API_KEY env var as an
  // authorized credential in ALL modes (interactive + headless), bypassing the
  // customApiKeyResponses approval list and OAuth login. This lets scripts on
  // Linux run with just an env key (or a custom ANTHROPIC_BASE_URL proxy).
  if (apiKeyEnv) {
    return {
      key: apiKeyEnv,
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    // Login-restriction removed: do not hard-fail when no credential is present
    // in CI/test. Fall through to returning { key: null, source: 'none' } so the
    // process can still start (callers handle the no-key case gracefully).
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  // Login-restriction removed: never enforce the enterprise forceLoginOrgUUID
  // organization lock so the CLI can be used without a Claude account / OAuth.
  return { valid: true }
}
